...
const fee = swapResult.fees?.find(
  (item) => item.token === "USDC",
);

const feeText = fee?.amount
  ? `${fee.amount} USDC`
  : "N/A";
...
const explorerUrl =
  ...
  (swapResult.txHash
    ? `https://testnet.arcscan.app/tx/${swapResult.txHash}`
    : "N/A");

setStatus(
  `SWAP COMPLETED\n\n` +
  `Input: ${amount} USDC\n` +
  `Output: ${swapResult.amountOut ?? "N/A"} EURC\n` +
  `Fee: ${feeText}\n\n` +
  `Transaction:\n${swapResult.txHash ?? "N/A"}\n\n` +
  `ArcScan:\n${explorerUrl}\n\n` +
  `Status: ${swapResult.progress?.substatus ?? "COMPLETED"}`,
